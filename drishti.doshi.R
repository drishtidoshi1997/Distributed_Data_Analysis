...
#setwd("~/Desktop/drish/Term 2/Coursework/DDA/EDA/sujeet(1)")

students.df <- read.csv(file = "student_dropout.csv",
                        header = T, sep = ",")
...
#   
#   # Pie chart
#   var <- students.df[,col_name]
#   xlab = col_name
#   title = paste("Pie Chart for", col_name)
#   df <- as.data.frame(table(var))
#   df2 <- df %>%
#     mutate(csum = rev(cumsum(rev(Freq))),
#            pos = Freq/2 + lead(csum, 1),
#            pos = if_else(is.na(pos), Freq/2, pos))
#   pie_chart <- ggplot(df, aes(x = "" , y = Freq, fill = var)) +
#     geom_col(width = 1, color=alpha("white",alpha = 0.3)) +
#     coord_polar(theta = "y") +
#     geom_label_repel(data = df2,
#                      aes(y = pos, label = paste0(round(100*Freq/sum(Freq),2), "%")),
#                      size = 3, nudge_x = 1, show.legend = FALSE,
#                      color=alpha("#888888", alpha = 1)) +
#     scale_fill_viridis(discrete = TRUE) +
#     guides(fill = guide_legend(title = xlab)) + ggtitle(title) +
#     theme_void()
#   
#   # Print the visualizations
#   print(paste("Frequency Table for", col_name))
#   print(freq_table_df)    
...
students.df.cleaned$Application.order[students.df.cleaned$Application.order == 0] <- mode_vskim(students.df.cleaned$Application.order)
...
student_tree <- students.df.cleaned[, !(names(students.df.cleaned) %in% c("Father.s.qualification", "Father.s.occupation"))]
student_tree
colnames(student_tree)

freq_table <- table(students.df.cleaned$Father.s.occupation)
low_freq_levels <- names(freq_table)[which(freq_table < 150)]
low_freq_levels

class(students.df.cleaned$Father.s.occupation)
str(students.df.cleaned)
# 7,13,19
colnames(students.df.cleaned)
str(students.df.cleaned)

set.seed(1234)
partitioned_data2 <- sample(2 , nrow(students.df.cleaned) , replace = TRUE , prob=c(0.8 , 0.2))
partitioned_data2
train2<- students.df.cleaned[partitioned_data2==1,]
train2
test2<- students.df.cleaned [partitioned_data2==2,]
test2

# Data partition
set.seed(1234)
partitioned_data <- sample(2 , nrow(students.df.cleaned) , replace = TRUE , prob=c(0.8 , 0.2))
partitioned_data
library(tree)
# Test and Train data
train<- students.df.cleaned[partitioned_data==1,]
train

test<- students.df.cleaned[partitioned_data==2,]
test

# Decision Tree with Party package
library(party)
decision_tree <- ctree(Target ~. , data=train)
decision_tree
par(mar=c(5,6,4,1)+.1)
plot(decision_tree)

# Adding control function to limit the size of the decsion tree
# Pruning the tree
decision_tree2 <- ctree(Target ~. , data=train, controls = ctree_control(mincriterion = 0.99, minsplit = 500))
decision_tree2
par(mar=c(1,1,1,1))
plot(decision_tree2)

decision_tree3 <- ctree(Target ~. , data=train, controls = ctree_control(mincriterion = 0.99, minsplit = 1000))
decision_tree3
par(mar=c(1,1,1,1))
plot(decision_tree3)
...
library(tree)
colnames(train)
#variables <- Target~ Age.at.enrollment +Curricular.units.1st.sem..credited. + Curricular.units.1st.sem..enrolled. + Curricular.units.1st.sem..evaluations.+   Curricular.units.1st.sem..approved +Curricular.units.1st.sem..grade.+Curricular.units.1st.sem..without.evaluations.+ Curricular.units.2nd.sem..credited.+Curricular.units.2nd.sem..enrolled. + Curricular.units.2nd.sem..evaluations. + Curricular.units.2nd.sem..approved. + Curricular.units.2nd.sem..grade.+Curricular.units.2nd.sem..without.evaluations.+ Unemployment.rate + Inflation.rate +GDP                                     
          


# train a decision tree
decision_tree4 <- tree(Target ~. , data = train2)
?tree
View(students.df)
...
# plot the tree
plot(tree_carseats_sales)
text(tree_carseats_sales, pretty = 0)

# prune the tree using cross-validation
cv_carseats_sales <- cv.tree(tree_carseats_sales, FUN=prune.misclass)
...
cv_carseats_sales_table <- data.frame(
  size = cv_carseats_sales$size,
  error = cv_carseats_sales$dev
)

# plot the cv_carseats_sales_table
plot(
  cv_carseats_sales_table,
  xaxt = 'n',
  yaxt = 'n'
)
axis(1, seq(1,max(cv_carseats_sales_table$size)))
axis(2, seq(50,150,5))

# select the tree size with the minimum error
pruned_tree_size <- cv_carseats_sales_table[which.min(cv_carseats_sales_table$error), 'size']

# prune the tree to the required size
pruned_tree_carseats_sales <- prune.misclass(tree_carseats_sales, best = pruned_tree_size)

# inspect the pruned tree
summary(pruned_tree_carseats_sales)

# plot the tree
plot(pruned_tree_carseats_sales)
text(pruned_tree_carseats_sales, pretty = 0)
...
# Misclassification error for the train data
 train_error <- table(predict(decision_tree) , train$Target)
print(train_error)
1-sum(diag(train_error)) / sum(train_error)

# Misclassification error for the train data

test_pred <- predict(decision_tree , newdata=test)
test_error <- table(test_pred , test$Target)
print(test_error)
1-sum(diag(test_error)) / sum(test_error)
